-- The remote D1 database was seeded with the "Jordan Reyes" placeholder
-- content before 0002_seed.sql was rewritten with Dylan's real content.
-- D1 tracks applied migrations by filename, so editing 0002_seed.sql after
-- the fact didn't retroactively re-run it there (it still worked for anyone
-- migrating a fresh database, like local dev). This migration clears the old
-- rows and re-inserts the current content so remote catches up.
--
-- project_tags cascades on both projects and tags deletes (see 0001_init.sql),
-- so deleting those two is enough to clear it too.
